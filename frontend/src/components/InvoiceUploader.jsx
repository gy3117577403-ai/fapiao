...
export default function InvoiceUploader({ disabled, uploading = false, onChange, compact = false, label = "上传发票" }) {
  return (
    <Stack spacing={0.75} alignItems={compact ? "flex-end" : "stretch"}>
      <Button component="label" size="small" startIcon={uploading ? <HourglassTopIcon /> : <CloudUploadIcon />} disabled={disabled || uploading}>
        {uploading ? "上传中" : label}
      ...
      {uploading ? (
        ...
        <StatusPill status="pending" label="未上传" icon={false} sx={{ display: compact ? { xs: "none", md: "inline-flex" } : "inline-flex" }} />
      )}
